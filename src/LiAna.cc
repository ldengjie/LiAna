...
bool LiAna::initialize()
{
    ...
    for( int i=0 ; i<6 ; i++ )
    {
        ...
        histName+="NoRed";
	    showermuonNumNoRed[i] = new TH1F(histName,"number of AD showermuon NoRed ",liveTimeSvc->nBins(),liveTimeSvc->startTime().AsDouble(),liveTimeSvc->endTime().AsDouble());
        ...
        histName+="NoRed";
        time2lastshowermuon4LiNoRed[i] = new TH1F(histName,"time2lastshowermuon Li",100000,0.,100.); 
	    ntupleSvc()->attach("FILE1/lidj",time2lastshowermuon4LiNoRed[i]);
    ...
    LiTree->Branch("promptT2Muon",promptT2Muon,"promptT2Muon[16]/F");
...
bool LiAna::FillLi(vector<PhyEvent*> EvtGroup)
{
    ...
    for( int i=0 ; i<6 ; i++ )
    {
        ...
        time2lastshowermuon4LiNoRed[i]->Fill(time2MuonBuf[EvtGroup[0]->m_det-1][0][i+10]);
    }
